(function () {
  ...
  var primaryButton = document.getElementById('downloadLms');
  var downloadButtons = [primaryButton].concat(Array.from(document.querySelectorAll('[data-download-lms]')));
  ...
  async function downloadLatest() {
    ...
    try {
      var response = await fetch('https://api.github.com/repos/MohamedFadlala/lms-releases/releases/latest', {
      ...
      var installer = (release.assets || []).find(function (asset) {
        return /^Holool-Laboratory-Manager-.*-Setup\.exe$/i.test(asset.name) && asset.state === 'uploaded';
      });
      if (!installer) throw new Error('Windows installer unavailable');
      var url = new URL(installer.browser_download_url);
      if (url.origin !== 'https://github.com' || !url.pathname.startsWith('/MohamedFadlala/lms-releases/releases/download/') || !url.pathname.toLowerCase().endsWith('.exe')) {
